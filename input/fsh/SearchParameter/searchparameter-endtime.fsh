// Shared End Time SearchParameter for both HealthcareService and PractitionerRole
...
* type = #number
* expression = "availableTime.select($this.availableEndTime.toString().replace(':','').substring(0,4).toInteger())"
